# `services.docx_tool_loader` — recovered API surface
...
- `get_markdown_to_docx(tools_dir: 'Path')` — 便捷函数：返回 markdown_to_docx 函数对象。
- `load_docx_export(tools_dir: 'Path') -> 'Any'` — 加载 tools/docx_export 模块并返回。结果会缓存。
