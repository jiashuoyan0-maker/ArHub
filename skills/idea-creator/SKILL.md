...
- **Large file handling**: For long reports like `IDEA_REPORT.md`, ALWAYS write using Bash heredoc in chunks instead of the Write tool. The Write tool may fail silently when content is too large (the tool call gets truncated, resulting in empty parameters). Use this pattern:
  ```bash
