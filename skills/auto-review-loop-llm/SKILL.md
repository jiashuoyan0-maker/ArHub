...
description: Autonomous research review loop using any OpenAI-compatible LLM API. Configure reviewer via ArHub settings page or environment variables. Trigger with "auto review loop llm" or "llm review".
...
The reviewer model is configured by the user in the ArHub settings page (reviewer API Key / Base URL / Model ID). These are injected as environment variables (`OPENAI_API_KEY`, `OPENAI_BASE_URL`, `REVIEWER_MODEL_ID`) automatically.
