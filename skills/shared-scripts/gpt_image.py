...
环境变量:
  ...
  GPT_IMAGE_BASE_URL — OpenAI-compatible 图片 API 地址（必须）
...
def main():
    ...
    if args.check:
        if api_key and api_base:
            ...
            print("GPT_IMAGE_AVAILABLE=NO (API key or base URL not set)")
            sys.exit(1)
    ...
    if not api_key:
        ...
        sys.exit(1)
    if not api_base:
        print("GPT_IMAGE_NOT_CONFIGURED: Set GPT_IMAGE_BASE_URL in settings")
        sys.exit(1)
