...
ArHub 当前已具备源码发布候选条件：后端、Agent、工作流、编辑器和 DOCX
链路均有可审查实现；30 个回归测试、70 路由导入、真实浏览器验收和 460 文件
开源审计均已通过。
...
- [ ] 启用 GitHub Secret scanning、Private vulnerability reporting 和分支保护
- [ ] 确认默认分支、Issue 模板和贡献流程符合公开项目计划

## 发布边界

源码 Release 可以继续准备。Windows 安装包 Release 暂不应发布，直到以下流程
可在干净机器上复现：

- Electron 与 Python runtime 的锁定和组装
- 前端模块化构建
- 依赖 SBOM 与第三方许可自动检查
- 安装包签名、校验和与升级回滚
- 干净 Windows 环境端到端测试
